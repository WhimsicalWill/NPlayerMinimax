...
impl ConnectFour {
    pub fn new(num_rows: usize, num_cols: usize, num_players: usize) -> Self {
        ConnectFour {
    ...
    pub fn is_row_win(&self, player: usize) -> bool {
        for row in self.state.get_board().iter() {
            for i in 0..self.num_cols - 3 {
                if self.is_sequence_win(&row[i..i+4], player) {
                    return true;
                }
            }
        }
    ...
    pub fn is_col_win(&self, player: usize) -> bool {
        for col in 0..self.num_cols {
            ...
            for i in 0..self.num_rows - 3 {
                if self.is_sequence_win(&col_elems[i..i+4], player) {
                    return true;
                }
            }
        }
    ...
    pub fn is_diag_win(&self, player: usize) -> bool {
        ...
        for i in 0..self.num_rows - 3 {
            for j in 0..self.num_cols - 3 {
                let diagonal: Vec<i32> = (0..4).map(|k| self.state.get_board()[i+k][j+k]).collect();
